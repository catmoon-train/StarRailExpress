...
public class PelicanHud {
    ...
    public static void register() {
        RoleHudRenderCallback.EVENT.register(ModRoles.PELICAN_ID, (context, tickCounter) -> {
            ...
            PelicanRoleData data = RoleData.getNullable(PelicanRoleData.class, client.player);
            if (data == null) return;
            ...
            Component progressLine = Component.translatable("hud.noellesroles.pelican.progress",
                    data.eatenCount, data.requiredEaten);
            ...
            // 冷却信息（在进度上方显示）
            int cooldownTicks = data.getRemainingCooldownTicks();
            if (cooldownTicks > 0) {
